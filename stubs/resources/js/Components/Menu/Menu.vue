<template>
    <Menu as="div" class="relative inline-block text-left" v-slot="{ open }" ref="menu">
        <div>
            <slot name="trigger" :open="open">
                <MenuButton>
                    <span class="inline-flex w-full items-center justify-center gap-x-4 rounded-md bg-white px-3 py-2 text-sm font-medium text-gray-700 
        ...
        <teleport :to="useFixed ? 'body' : null">
            ...
            >
                <MenuItems class="absolute overflow-hidden z-10 mt-1 py-0.5 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none -translate-x-full"
                    :style="style"
                >
                    <slot />
                </MenuItems>
