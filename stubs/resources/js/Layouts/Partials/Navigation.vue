...
<template>
    <Disclosure as="nav" class="border-b border-b-slate-200" v-slot="{ open }">
                    ...
                    <div class="flex justify-between w-full">
                        <div class="hidden sm:flex sm:ml-6 space-x-8" v-if="$page.props.auth.user">
                            <Link :href="route('home')" 
                                ...
                                :class="{ '!border-blue-500': route().current('home') }"
                            >
                                Home
                            </Link>
                            <Link :href="route('dashboard')" 
                                class="inline-flex items-center border-b-2 border-transparent hover:border-gray-200 text-sm font-medium text-gray-900"
                                :class="{ '!border-blue-500': route().current('dashboard') }"
                            >
                                Dashboard
                            </Link>
                        </div>
                        <div class="hidden sm:flex sm:ml-6 space-x-8" v-else>
                            <Link v-if="$page.props.features.registration" 
                ...
                <div class="absolute inset-y-0 right-0 flex items-center space-x-3">
                    <Menu as="div" class="relative mr-3" v-if="$page.props.auth.user">
                        <MenuButton class="flex items-center text-sm space-x-3">
                            ...
                            <img :src="$page.props.auth.user.avatar" class="h-8 w-8 rounded-full">
                        </MenuButton>
                        <MenuItems class="absolute right-0 z-10 mt-2 w-48 bg-white border border-b-slate-200 focus:outline-none">
                            <MenuItem v-slot="{ active, close }">
                                <Link v-if="$page.props.features['update-profile-information']" 
                            ...
                            <MenuItem v-slot="{ active, close }">
                                <Link v-if="$page.props.features.security" 
                            ...
                            <MenuItem v-slot="{ active }">
                                <button @click="logout" class="block text-left w-full px-4 py-2 text-sm text-gray-900" :class="{ 'bg-gray-100': active }">
                                    Sign out
                                </button>
                            </MenuItem>
                        </MenuItems>
        ...
        <DisclosurePanel class="sm:hidden">
            ...
            <div class="space-y-1 pb-3" v-if="!$page.props.auth.user">
                <Link v-if="$page.props.features.registration" 
                    :href="route('auth.register')" 
                    preserve-scroll 
                    preserve-state 
                ...
                <Link :href="route('auth.login')" 
                    preserve-scroll 
                    preserve-state 
                    :only="['modal']" class="block py-2 px-8 font-medium text-gray-900">
                    Sign in
                </Link>
            </div>
        </DisclosurePanel>
